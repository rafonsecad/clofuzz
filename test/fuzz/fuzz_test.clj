(ns fuzz.fuzz-test
  (:require [clojure.test :refer :all]
            [fuzz.core :as fuzz]
            [fuzz.terminal :as fuzz-t]
            [fuzz.backup :as fuzz-bk]
            [ring.adapter.jetty :as jetty]))

(def MockTerminal 
  (reify fuzz-t/TerminalProtocol
    (handle [_])
    (log [_ msg])
    (stop [_])))

(def MockBackup
  (reify fuzz-bk/BackupProtocol
    (init [_])
    (create-state [_ state stats])
    (update-state [_ id stats])
    (save-match [_ id match])))

(defn ok-handler [request]
  (case (:uri request)
    
    "/ok"
    {:status 200 :body "ok" :content-length 2}

    "/ok-no-content-length"
    {:status 200 :body "ok"}

    {:status 404 :body "not found"}))

(defn post-handler [{:keys [uri request-method] }]
  (if (and (= request-method :post) (= uri "/ok"))
    {:status 200 :body "ok"}
    {:status 404 :body "not found"}))

(defn start-mock-server [handler]
  (let [random-port
        (+ (rand-int 16383) 49152)
        
        server
        (jetty/run-jetty handler {:port random-port :join? false})
        
        url-fuzz
        (str "http://localhost:" random-port "/FUZZ")
        
        _
        (println "Creating mock server on port" random-port)] 
    {:port random-port
     :server server
     :url-fuzz url-fuzz}))

(deftest filter-by-status-code-test
  (testing "OK response at path /ok: one match for status code 200"
    (let  [ 
           mock-server 
           (start-mock-server ok-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "ok")

           response
           (fuzz/validate-request request [200] [] [] "ok" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 200))
     (is (= response {:status 200 :word "ok" :length 2}))))

  (testing "NOT FOUND response at path /guess: no matches for 200 status code"
    (let  [
           mock-server 
           (start-mock-server ok-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "guess")

           response
           (fuzz/validate-request request [200] []  [] "guess" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
      (is (= (:status request) 404))
      (is (= response nil)))))

(deftest no-content-length-response-test
  (testing "OK response at path /ok-no-content-length: one match with content-length of 2"
    (let  [
           mock-server 
           (start-mock-server ok-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "ok-no-content-length")

           response
           (fuzz/validate-request request [200] [] [] "ok-no-content-length" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 200))
     (is (= response {:status 200 :word "ok-no-content-length" :length 2})))))

(deftest filter-out-by-content-length-test
  (testing "filter out content-length of 9 at path /guess: no matches found"
    (let  [
           mock-server 
           (start-mock-server ok-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "guess")

           response
           (fuzz/validate-request request [404] [] [9] "guess" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 404))
     (is (= response nil)))))  

(deftest post-method-test
  (testing "scanning with post method at path /ok : one match found"
    (let  [           
           mock-server 
           (start-mock-server post-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "POST" {} false "ok")

           response
           (fuzz/validate-request request [200] [] [] "ok" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 200))
     (is (= response {:status 200 :word "ok" :length 2}))))

  (testing "scanning with get method at path /ok : no matches found"
    (let  [           
           mock-server 
           (start-mock-server post-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "ok")

           response
           (fuzz/validate-request request [200] [] [] "ok" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 404))
     (is (= response nil)))))

(deftest exlusion-by-status-code-test
  (testing "OK response at path /ok: exclude status code 200"
    (let  [           
           mock-server 
           (start-mock-server ok-handler)

           request
           (fuzz/mk-request (:url-fuzz mock-server) "GET" {} false "ok")

           response
           (fuzz/validate-request request [200] [200] [] "ok" MockTerminal MockBackup 1)

           _
           (.stop (:server mock-server))]
     (is (= (:status request) 200))
     (is (= response nil)))))

;(run-tests)
